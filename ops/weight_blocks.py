...
class FFGBlock(WeightBlock):
    ...
    def sample_mean_and_var(self, particles):
        mean = self._mean
        std = self._std
...
class MVGBlock(WeightBlock):
    def __init__(self, idx, shape, coeff, eta):
        ...
        self.nobias_uc = tf.get_variable(
            'w'+str(idx)+'nobias_uc',
            initializer=1e-3 * tf.eye(self._n_in-1),
            trainable=False
        )
    ...
    def sample_mean_and_var(self, particles, nobias_flag=False):
        mean = self._mean
        v_c = self._v_c
    ...
    def update(self, block):
        ...
        updates_op = [self._u_c.assign(new_uc), self._v_c.assign(new_vc), self.nobias_uc.assign(new_uc[:-1,:-1])]
